...
| snapshot vs delta identity (F0-02) | `event_kind` ∈ {`snapshot`, `delta`, `reconstructed`}. `is_complete_snapshot = 1` on two kinds of row: a `subscribed/order_book` message that carries a `nonce` (`event_kind = snapshot`, exchange-originated), and a `reconstructed` top-5 (`top_levels = 5`) emitted from the in-memory `BookState` at most once per `reconstructed_snapshot_interval_ms = 1000` while the book is synced, tagged with the last applied `nonce`. `delta` rows are always `0`. Offline code that wants only exchange-originated snapshots must filter on `event_kind`, not on `is_complete_snapshot` alone. |
