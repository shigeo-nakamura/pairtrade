...
| `book_runtime --validate --calendar <json>` | how a calendar is checked before it is installed (bot-strategy#952). `--validate` builds the scheduler, so shape and `deny_unknown_fields` at both levels, `calendar_version`, RFC 3339 timestamps, unique `decision_key`s, flatten-outside-the-signal-window and non-overlap are all enforced by the same code the service starts with, against the config's own `signal_grace_secs`. `--calendar` points it at a not-yet-installed file, which is what `install_book_runtime.sh` validates before it promotes anything; without the flag the runtime's own `schedule.calendar_path` is used. Rust CI runs the same build over the committed config/calendar pair |
| `scripts/test_exdiv_signal_producer.py` | unit tests (synthetic logger rows) |
| `deploy/book-runtime-exdiv-lighter.service` | runtime unit, status → `s3://debot-dashboard/debot/status/book-exdiv-lighter/` |
