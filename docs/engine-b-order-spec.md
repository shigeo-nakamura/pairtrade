...
- `spread = Some(-1)` ("IOC" sentinel) is **degraded to GTT** by the
  ...
  `create_order_taker_ioc`, which is what §3.2 now uses. Getting §6.3's
...
| G-3 | ~~Entry/exit is MARKET+IOC with a **±20 % protection price**, not the doc's "marketable limit ≤ 50 bps from mid" (§6.3). No limit-IOC path exists in the connector for Lighter.~~ **Closed** by dex-connector#88 + the pairtrade v4.7.22 bump and wiring (bot-strategy#918): `create_order_taker_ioc` sends LIMIT + TIF_IOC bounded `ENGINE_B_LIVE_SLIPPAGE_BPS` (default 50) from the touch, tick-rounded inward. | entry, exit | was low anyway: SNDK does ~$15 M/day with ~1.3 bps spread; observed top-5 depth ≈ $83 k vs a $100 order. The bound matters for the torn-book tail, not the median fill | verify on the first live cycle that a fill occurs at all at 50 bps; widen only with evidence, never back to a protection price |
