...
#
# BOOK_SIGNAL_MAX_AGE_SECS (from <instance>.fetch.env, generated by
# install_book_runtime.sh out of the validated config) bounds how old a
# downloaded signal may be, matching signal.rs's own check; unset skips
# only that one check, since the runtime remains authoritative.
#
...
if ! SUMMARY=$(BOOK_SIGNAL_MAX_AGE_SECS="${BOOK_SIGNAL_MAX_AGE_SECS:-}" python3 - "$TMP" <<'PY'
