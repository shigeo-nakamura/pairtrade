#!/bin/bash
# Production wrapper for the multi-pair observe-only data collector
# (bot-strategy#560; extends the #519 second-pair study).
#
# - observe_only + dry_run: never places orders, real or simulated. Its sole
#   deliverable is /opt/debot-shadow/market_data_pairscreen_*.jsonl for the
#   #560 second-pair backtest (BTC/ETH control + DOT/ETH, AVAX/BTC, LTC/BTC,
#   SOL/ETH candidates, real ~5s cadence + real L1 quotes).
...
# - State lives under /opt/debot-shadow/ so its history snapshots, risk_state,
#   etc. never collide with /opt/debot/, /opt/debot-canary/, or /opt/debot-sol/.

set -eu

ENV_DIR="${DEBOT_ENV_DIR:-/opt/debot/scripts}"
STATE_DIR="${DEBOT_SHADOW_STATE_DIR:-/opt/debot-shadow}"
...
export DEBOT_STATUS_ID=debot-pair-shadow
export PAIRTRADE_CONFIG_PATH=/opt/debot/configs/pairtrade/debot-pair-shadow.yaml
...
# keeps the path disjoint from main/canary/sol.
